...
This article provides details on the MEWS PMS to QuickBooks accounting integration. The accounting integration transfers data from MEWS to QuickBooks via an API-connection.
...
**PART 4**: (Accounting) information necessary to build the MEWS to QuickBooks integration

**PART 5**: Reconciliation of accounting data sent to QuickBooks

**PART 6**: Other features and configurable items

# PART 1: General setup process of the integration

The start-to-end process of the MEWS to QuickBooks accounting integration goes as follows:

**1.1 Request the QuickBooks integration from the MEWS Marketplace**
...
**1.6 Omniboost to send test entries to your QuickBooks environment and verification of test entries by your accounting department**

Once there is accounting data available in the MEWS Accounting report, the integration is able to send over test accounting entries to your QuickBooks environment.
...
Once your accounting department has verified the test entries and these are correct, your Omniboost contact person can activate the accounting integration. This means that the integration will fully automatically send over the accounting data to your QuickBooks environment on a daily basis.
...
In order to correctly transfer the accounting data from MEWS to QuickBooks, several pieces of accounting information are required. Please find those items below and provide them to your Omniboost contact person.
...
Accounts Receivable information consists of sales invoices and the information of the corresponding debtor accounts. The integration is able to send over invoices\* to QuickBooks via two ways:

- As sales entries (including debtor information)
- As part of general journal entries

**\*By default, invoices are sent over to QuickBooks as Sales entries.** The reason for this is that VAT amounts are part of the journal entries that are sent over to QuickBooks. If the connection would send over actual invoices with VAT on the invoice lines to QuickBooks, VAT would be accounted for twice in the accounting platform. In order to prevent this from happening, the connection sends over a Sales entry rather than an actual Invoice.

Only within the Closed Bills and Invoices accounting flow, invoices are sent over as actual invoices to QuickBooks.
...
**4.4 Please activate the QuickBooks Oauth-link as provided by your Omniboost Contact person.**

The MEWS PMS to QuickBooks accounting integration is based on an API-connection which means that accounting data can be automatically pushed into your QuickBooks environment. **Please note that** all entries which are pushed into your QuickBooks environment, are **draft** entries. This means that you are always able to modify or remove these entries in case this is necessary.

In order to establish this API-connection, your Omniboost contact person will reach out to you with the request to activate a QuickBooks Oauth-link. **Please note that activation of this link** only enables the Omniboost accounting integration to push data into your QuickBooks environment. It does **not** give anyone from Omniboost access to the environment itself.
...
By default, the integration posts VAT and/or Tax amounts to VAT/Tax ledger accounts which exist in the chart of accounts of your QuickBooks environment. These ledger accounts are entered by Omniboost in the integration's mapping. We therefore ask you to provide us with a ledger account code for each VAT and/or Tax rate applicable to your hotel revenues. It is also possible to use the same ledger account code for multiple VAT/Tax rates.
...
In case you want the integration to send over Accounts Receivable information to QuickBooks (i.e. invoices along with debtor information), we can let the accounting integration generate debtor numbers.
...
| **QuickBooks Oauth link** |
| **VAT and/or Tax ledger accounts codes** |
...
- Mode: Closed
- Date filter based on the closed date

In case the integration uses a Consumed accounting flow, you should run the Accounting report as follows:

- Mode: Consumed
- Date filter based on the consumed date

**In case the integration runs on a Closed accounting flow based on Bills and Invoices,**

MEWS accounting data can be reconciled with the data in QuickBooks as sent over by the Omniboost accounting integration **by looking at the MEWS _Bills & Invoices overview_.**

Please note that the MEWS _Accounting_ _report_ shows revenues (and payments) based on an accounting category level. The _Bills and invoices_ overview provides the same information; however, it does so based on the level of individual bills and invoices.
...
**1. Sending over credit card commission fees separately to QuickBooks**
...
In case the default currency in your MEWS PMS environment differs from the default currency in your QuickBooks environment, please let us know. We can apply a currency conversation rate in the accounting integration. Please note that this is a fixed rate conversation and not a real-time one.