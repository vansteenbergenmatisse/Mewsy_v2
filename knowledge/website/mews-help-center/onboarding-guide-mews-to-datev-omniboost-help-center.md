...
| --- | --- |
| Umsatz | The revenue or payment (or deposit) amount as shown in the Accounting report |
| Soll/Haben | Whether the amount is posted on the Debit or Credit side |
| Konto | The ledger account to which the amount is posted |
| Gegenkonto | The offsetting account (i.e. Guest ledger account) |
| Belegdatum | The date of the entry |
| Belegfeld 1 | Usually this is the bill number as seen in the Bills & Invoices report in MEWS. |
| Buchungstext | This is the description. For revenues, we usually use the Accounting category name (from MEWS) as the description. For payments and deposits, we usually use the client or company name as the description. |
| KOST1 Kostenstelle | These are cost center codes. In case you have entered values in the 'Cost center code' column in the overview of accounting categories in MEWS, the accounting integration will include these values in the Kostenstelle column. |

**Reconciling the Debtor file (C file)**

The Debtor file is generated in case invoices have been issued in MEWS. You can check whether invoices has been issued **by looking at the _Bills & Invoices report_.**
