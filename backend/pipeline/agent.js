 ...
 *
 *   Step 4 — Answer
 *     Pass message + context to claude.js, which calls the API and returns a reply.
 *
...
import { chat, selectRelevantFiles } from './claude.js';
import {
  getSession,
  updateContext,
} from './session.js';
...
// Sentinel value passed to claude.js when no knowledge files matched at all.
const BASIC_MODE = '__BASIC_MODE__';

// Sentinel prefix passed to claude.js when docs were found but confidence is low.
...
async function loadKnowledgeFiles(pages) {
  const results = await Promise.all(
    pages.map(async (page) => {
      try {
        return await readFile(join(ROOT, page.path), 'utf-8');
      } catch (err) {
        console.warn(`[agent] could not load ${page.path}: ${err.message}`);
        return null;
      }
    })
  );
...
function shouldSkipRouting(userMessage, sessionContext, isFirstMessage) {
...
function extractPreviousQuestion(response) {
...
// Called by server.js for every incoming chat message.
...
export async function handleMessage(sessionId, userMessage) {
  ...
  try {
    ...
    console.error(`[agent] could not load manifest: ${err.message}`);
  }
  ...
  if (skipRouting && context.lastLoadedDocIds.length > 0) {
    ...
    try {
      ...
      console.error(`[agent] selectRelevantFiles failed: ${err.message}`);
    }
  }
  ...
  let knowledgeContent = BASIC_MODE;
