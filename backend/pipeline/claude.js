 ...
 *
 *   Block 1 — Base prompt (Mewsy's rules and personality from prompts/system.js)
 *     This text is identical on every single call. Marked with cache_control
 *     so Anthropic stores it server-side for 5 minutes.
 *
 *   Block 2 — Knowledge content (the .md files loaded by agent.js)
...
import { getHistory, addToHistory } from './session.js';
import {
  ROUTER_MAX_DOCS,
...
// Matches the sentinel values set in agent.js.
...
export function buildSystemPrompt(knowledgeContent = null, sessionContext = null) {
...
export async function chat(sessionId, userMessage, knowledgeContent = null, sessionContext = null) {
...
export async function selectRelevantFiles(pages, userMessage, conversationHistory = []) {
...
- NEVER return explanations, NEVER return document names, ONLY return the JSON object`,
    ...
    try {
      ...
      console.error(`[claude] selectRelevantFiles JSON parse failed: ${parseErr.message} — raw: ${raw}`);
      return { indices: [], confidence: 0.0 };
    }
  } catch (err) {
    console.error(`[claude] selectRelevantFiles failed: ${err.message}`);
    return { indices: [], confidence: 0.0 };
  }
}
