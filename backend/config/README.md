...
This folder has one job: store every tunable number or setting for Mewsie in one place. If you want to change how Mewsie behaves — like how many documents it loads at once, or when it asks a clarifying question — you change it here, not buried inside the logic files.
...
| `ROUTER_CONFIDENCE_THRESHOLD` | If confidence drops below this, Mewsie asks a clarifying question instead of guessing (0.90 = 90%) |
...
| `MAX_CLARIFY_ROUNDS` | After this many clarifying questions with no resolution, Mewsie just gives its best answer (3) |
| `BUTTON_MAX` | Maximum number of option buttons to show at once (7) |
| `FRUSTRATION_THRESHOLD` | After this many frustrated messages, Mewsie offers to create a support ticket (3) |
