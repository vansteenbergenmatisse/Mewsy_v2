...
async function firecrawlScrape(url: string): Promise<FirecrawlData> {
...
async function filterArticleLinks(links: string[]): Promise<string[]> {
  ...
  try {
    ...
    logger.warn(`Article link filtering failed: ${(err as Error).message} — using all links`);
 ...
 */
export async function scrapeMulti(page: FetchPage, forceSync: boolean, existingEntries: Record<string, ExistingEntry>): Promise<void> {
  const { id, label, url } = page;
  const outDir = join(process.cwd(), config.knowledgeDir, 'website', id);

  // Step 1: Scrape index page
  let indexData: FirecrawlData;
  try {
