 ...
 */
function preprocessConfluenceContent(html) {
  return html
    ...
    .replace(
      ...
      (_, code) => `<pre><code>${code.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</code></pre>`
    )
 ...
 */
function confluenceAxiosHeaders() {
...
async function processPage(page, pathSegments, folderSlug, folderId, forceSync, existingEntries, depth = 0) {
  ...
  try {
    ...
    logger.error(`Failed to get children of Confluence page "${title}" (${page.id}): ${err.message}`);
    return;
  }
 ...
 */
export async function scrapeConfluence(folder, forceSync, existingEntries) {
  ...
  try {
    ...
    logger.error(`Failed to fetch Confluence folder ${id}: ${err.message}`);
    return;
  }
