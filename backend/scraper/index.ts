/**
 * index.ts — Scraper entry point and cron scheduler
 *
 * Two modes:
 *   tsx backend/scraper/index.ts --force-sync   → full sync, then exit
 *   tsx backend/scraper/index.ts                → start cron jobs, run forever
 *
...
// Shape of a fetch_sources.json page entry
interface FetchPage {
  id: string;
  label: string;
  url: string;
...
interface FetchFolder {
  id: string;
  label: string;
}

// Shape of the full fetch_sources.json file
interface FetchSources {
  confluence: { syncIntervalMinutes: number; folders: FetchFolder[] };
  website: { syncIntervalMinutes: number; pages: FetchPage[] };
}

// ── Helpers ───────────────────────────────────────────────────────────────────

function readFetchSources(): FetchSources {
  ...
  return JSON.parse(readFileSync(path, 'utf8')) as FetchSources;
}
 ...
 */
function deleteFileAndPruneEmptyDirs(filePath: string): void {
 ...
 */
function runDeletionCheck(sources: FetchSources): void {
...
async function runSync(type: 'website' | 'confluence'): Promise<void> {
  ...
  const existingEntries: Record<string, { content_hash?: string }> = {};
  ...
  try {
    ...
    logger.error(`Sync failed (${type}): ${(err as Error).message}`);
  } finally {
