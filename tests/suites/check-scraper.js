...
export async function checkScraper({ pass, fail, skip, results }) {
  // ── slugify ──────────────────────────────────────────────────────────────
  const { slugify } = await import(`${ROOT}/backend/scraper/utils/slugify.js`);
  ...
  const numericResult = slugify('99999');
  ...
  // ── hash ────────────────────────────────────────────────────────────────
  const { sha256 } = await import(`${ROOT}/backend/scraper/utils/hash.js`);

  const h1 = sha256('hello world');
  const h2 = sha256('hello world');
  const h3 = sha256('different input');
  ...
  // ── manifest read/write ─────────────────────────────────────────────────
  const { readManifest, writeManifest } = await import(`${ROOT}/backend/scraper/pipeline/manifest.js`);
  ...
  const testData = { pages: { test: { title: 'T', description: 'D', keywords: ['a'], path: 'knowledge/mews.md' } } };

  try {
    writeFileSync(testManifestPath, JSON.stringify(testData, null, 2));
    const readBack = JSON.parse(readFileSync(testManifestPath, 'utf8'));
    ...
    fail('manifest read/write', err.message);
    results.push({ ok: false });
  } finally {
  ...
  // ── manifest validation ──────────────────────────────────────────────────
  const { upsertEntry } = await import(`${ROOT}/backend/scraper/pipeline/manifest.js`);
