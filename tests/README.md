...
| 6 | [check-session.ts](suites/check-session.ts) | Session creation (incl. empty `clarificationBundles`, empty `qaLog`, null `originalQuestion`, `postAnswerMode: false`, `postAnswerSignal: null`, `answerContract: null`, empty `qaLogSnapshot`, `postAnswerClarifyUsed: false`), history append and retrieval, history trimming to max pairs, context update, non-destructive patch, TTL expiry via `cleanSessions` | — | — |
